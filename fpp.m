...
% Last Modified by GUIDE v2.5 27-Oct-2015 20:14:08

% Begin initialization code - DO NOT EDIT
% Примичание: данная функция является стандартной
gui_Singleton = 1;
...
% Выполняется пред тем как FPP станет вилимым
function fpp_OpeningFcn(hObject, eventdata, handles, varargin)
% Это функция не имеет выходных переменных смотри OutputFunc
% hObject    управление картинкой
% eventdata  reserved - to be defined in a future version of MATLAB
%---выше стандартная процедура для совместимости между версиями MatLab
% handles   для пользовательских данных (see GUIDATA)
% varargin   для командной строки (смотри VARARGIN)

%Выбор по умолчанию выхода для FPP
handles.output = hObject;

% Обновление управляемой структуры
...
axes(handles.axes1) % выбор свойств осей
...
% hObject   управление рисунком
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% в принципе все функции имееют стандартный набор входный параметров
...
axes(handles.axes2) %  выбор свойств осей
...
axis([1 100 1 100]);
% hold on
h=msgbox('Нарисуйте стену используя ЛКМ для указания начала и ПКМ для конца');
uiwait(h,5);
if ishandle(h)==1
    delete(h);
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
w = str2double(get(handles.edit1,'String'));
h=msgbox('Укажите координаты цели с помошью ЛКМ');
uiwait(h,5);
if ishandle(h)==1
    delete(h);
end
...
hold off
    ...
    msgbox('Выбирете создать новую карту');
end
...
set(handles.text2,'String','Присутствует локальный минимум')
end

if(NoSol==1)
%    display('No Solution')
    set(handles.text2,'String','Решение задачи не найдено')
else
    if(LMF==1)
%    display('Local Minimum exists')
    set(handles.text2,'String','Присуствует локальный минимум')
    else
%    display('A Solution exist')
    set(handles.text2,'String','Путь найден')
    end
end
...
setappdata(0,'posf',posf);
    ...
    msgbox('Выбирете координаты цели');
end
